		...
		switch (noteTypeCheck)
		{
			...
			default:
				switch(noteType)
				{
					case 2:
					{
						frames = Paths.getSparrowAtlas('hacked/NOTE_guest','guest');
		...
		if (mustPress)
		{
			...
			else
			{
				if (noteType != 2)
					...
					if (strumTime > Conductor.songPosition - (Conductor.safeZoneOffset * 0.5) && strumTime < Conductor.songPosition + (Conductor.safeZoneOffset * 0.5))
						canBeHit = true;
					else
						canBeHit = false;
			}
