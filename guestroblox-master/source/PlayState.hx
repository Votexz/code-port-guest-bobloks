...
class PlayState extends MusicBeatState
{
	...
	var guestNoteSkin:String = 'NOTE_assets';

	public var iconP1:HealthIcon; //making these public again because i may be stupid
	public var iconP2:HealthIcon; //what could go wrong?
	public var camHUD:FlxCamera;
	private var camGame:FlxCamera;
	public var vig:FlxSprite = new FlxSprite(0, 0);
	...
	override public function create()
	{
		...
		{
			case 'song-simulator':
				dialogue = CoolUtil.coolTextFile(Paths.txt('song-simulator/dialogue'));
				usesDialogue = true;
			case 'respawn':
				dialogue = CoolUtil.coolTextFile(Paths.txt('respawn/dialogue'));	
				usesDialogue = true;
			case 'banned':
				dialogue = CoolUtil.coolTextFile(Paths.txt('banned/dialogue'));
				endDialogue = CoolUtil.coolTextFile(Paths.txt('banned/dialogue2'));
				usesDialogue = true;
				usesEndDialogue = true;
			case 'possessed':
				dialogue = CoolUtil.coolTextFile(Paths.txt('possessed/dialogue'));	
				usesDialogue = true;		
		}
		...
		switch(stageCheck)
		{
			...
			case 'roblos':
			{
				defaultCamZoom = 0.9;
				curStage = 'roblox';
				var bg:FlxSprite = new FlxSprite(-3.2, -3.2).loadGraphic(Paths.image('background123', 'guest'));
				bg.updateHitbox();
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				add(bg);
			...
			{
				defaultCamZoom = 0.9;
				curStage = 'spookyroblox';
				var bg:FlxSprite = new FlxSprite(-2.95, -183.4).loadGraphic(Paths.image('background4', 'guest'));
				bg.updateHitbox();
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;
				...
				vig.loadGraphic(Paths.image('hacked/spookyhackedlmao','guest'));
				vig.screenCenter(X);
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'guest':
				dad.setPosition(438.7, 376.55);
				camPos.set(dad.getGraphicMidpoint().x + 100, dad.getGraphicMidpoint().y + 40);
			case 'guest-final':
				dad.setPosition(486.1, 191.1);
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'roblox':
				boyfriend.setPosition(970.35, 440);
				gf.setPosition(570.2, 126.4);
			case 'spookyroblox':
				boyfriend.setPosition(970.35, 440);
				gf.setPosition(570.2, 126.4);	
		}
		...
		if(SONG.song.toLowerCase() == 'possessed')
		{
			healthBar.createFilledBar(0xFF000000, 0xFF31B0D1);
		}
		else
		{
			healthBar.createFilledBar(0xFFAFAFAF, 0xFF31B0D1);
		}
		// healthBar
		add(healthBar);
		...
		kadeEngineWatermark.setFormat(Paths.font("vcr.ttf"), 16, FlxColor.WHITE, RIGHT, FlxTextBorderStyle.OUTLINE,FlxColor.WHITE);
	...
	function startEndDialogue(?dialogueBox:DialogueBox):Void
	{
		inCutscene = true;
		add(dialogueBox);
	}
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					case 'normal':
						if (player == 0 && curSong == 'Possessed')
							babyArrow.frames = Paths.getSparrowAtlas(guestNoteSkin);
						else 
	...
	{
		//penis
		var funiscreencover = new FlxSprite();
		funiscreencover.frames = Paths.getSparrowAtlas('screen');
		funiscreencover.animation.addByPrefix('opening', 'opening', 24, false);
		funiscreencover.screenCenter(X);
		funiscreencover.cameras = [camHUD];
		add(funiscreencover);

		funiscreencover.animation.play('opening');
		
		funiscreencover.animation.finishCallback = function(pog:String)
		{
			trace('ended');
			remove(funiscreencover);
		}
	}
	...
	override public function update(elapsed:Float)
	{
		...
		if (dad.curCharacter == "guest-final"){
	...
	*/
	...
	override function beatHit()
	{
		...
		if (curSong == 'Possessed' && curStep == 446)
		{
			trace('nae nigga nae nae nigga');
		}
